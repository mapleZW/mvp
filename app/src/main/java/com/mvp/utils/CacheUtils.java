...
public class CacheUtils {
    ...
    public static File getFileDirectory(Context context) {
        ...
        if (appCacheDir == null) {
            String cacheDirPath = "/data/" + context.getPackageName() + "/files/";
    ...
    public static File getCacheDirectory(Context context, boolean preferExternal, String dirName) {
        ...
        if (appCacheDir == null) {
            String cacheDirPath = "/data/" + context.getPackageName() + "/cache/";
            LogUtils.d("Can't define system cache directory!", cacheDirPath);
            appCacheDir = new File(cacheDirPath);
        }
        return appCacheDir;
    }
    ...
    private static File getExternalCacheDir(Context context, String dirName) {
        ...
        if (!appCacheDir.exists()) {
            if (!appCacheDir.mkdirs()) {
                LogUtils.d("onResponse", "Unable to create external cache directory");
                return null;
            }
            try {
                new File(appCacheDir, ".nomedia").createNewFile();
            } catch (IOException e) {
                LogUtils.d("onResponse", "Can't create \".nomedia\" file in application external cache directory");
            }
        }
        return appCacheDir;
    }
